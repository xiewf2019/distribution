...
def experience(exps,seuil,workloads , tachePool, r_range,  prob_reussi, nb_exp, puni,  stra = True, insere = False, timeRange = [9,18],prob_tache = [7, 2.5, 0.5],relation_tache = True):
	...
	for r in r_range:
		...
		for i in range(10):
			...
			l1 += compterMission(DistributionSansRecuperation, simulation, pp1[0], tp[0], prob_reussi, nb_exp, puni,  nonstra , insere, timeRange ,prob_tache ,relation_tache )[0]
			l2 += compterMission(DistributionAvecRecuperationBas, simulation, pp1[1], tp[1], prob_reussi, nb_exp, puni, nonstra , insere, timeRange ,prob_tache ,relation_tache)[0]
			l3 += compterMission(DistributionSansRecuperation, simulation, pp1[2], tp[2], prob_reussi, nb_exp, puni, stra , insere, timeRange ,prob_tache ,relation_tache)[0]
			l4 += compterMission(DistributionAvecRecuperationBas, simulation, pp1[3], tp[3], prob_reussi, nb_exp, puni, stra , insere, timeRange ,prob_tache ,relation_tache)[0]
...
def affichage_moyen_stdev_v_stra_recuperation(r_range, moyen, stdev, listAlgo):
